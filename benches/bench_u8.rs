...
fn argminmax_u8_random_array_long(c: &mut Criterion) {
    ...
    c.bench_function("impl_u8_argminmax", |b| {
        b.iter(|| black_box(data.argminmax()))
    });
    c.bench_function("impl_u8_argmin", |b| b.iter(|| black_box(data.argmin())));
    c.bench_function("impl_u8_argmax", |b| b.iter(|| black_box(data.argmax())));
}
