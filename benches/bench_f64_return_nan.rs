...
fn nanargminmax_f64_random_array_long(c: &mut Criterion) {
    let n = config::ARRAY_LENGTH_LONG;
    let data: &[f64] = &utils::get_random_array::<f64>(n, f64::MIN, f64::MAX);
    c.bench_function("scalar_nanargminmax_f64", |b| {
    ...
    if is_x86_feature_detected!("avx2") {
        c.bench_function("avx2_nanargminmax_f64", |b| {
            b.iter(|| unsafe { AVX2::argminmax(black_box(data)) })
        });
    }
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    if is_x86_feature_detected!("avx512f") {
        c.bench_function("avx512_nanargminmax_f64", |b| {
...
criterion_group!(benches, nanargminmax_f64_random_array_long,);
