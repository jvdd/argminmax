...
mod avx2 {
    ...
    impl SIMD<u32, __m256i, i32, __m256i, __m256i, LANE_SIZE> for AVX2 {
        ...
        #[inline(always)]
        unsafe fn _reg_to_arr_values(_: __m256i) -> [u32; LANE_SIZE] {
...
mod sse {
    ...
    impl SIMD<u32, __m128i, i32, __m128i, __m128i, LANE_SIZE> for SSE {
        const INITIAL_INDEX: __m128i = unsafe { std::mem::transmute([0i32, 1i32, 2i32, 3i32]) };
        const MAX_INDEX: usize = i32::MAX as usize;

        #[inline(always)]
        unsafe fn _reg_to_arr_values(_: __m128i) -> [u32; LANE_SIZE] {
            // Not used because we work with i32ord and override _get_min_index_value and _get_max_index_value
            unimplemented!()
        }
...
mod avx512 {
    ...
    impl SIMD<u32, __m512i, i32, __m512i, u16, LANE_SIZE> for AVX512 {
        ...
        #[inline(always)]
        unsafe fn _reg_to_arr_values(_: __m512i) -> [u32; LANE_SIZE] {
...
mod neon {
    ...
    impl SIMD<u32, uint32x4_t, u32, uint32x4_t, uint32x4_t, LANE_SIZE> for NEON {
        const INITIAL_INDEX: uint32x4_t = unsafe { std::mem::transmute([0u32, 1u32, 2u32, 3u32]) };
        const MAX_INDEX: usize = u32::MAX as usize;

        #[inline(always)]
        unsafe fn _reg_to_arr_values(reg: uint32x4_t) -> [u32; LANE_SIZE] {
        ...
        #[inline(always)]
        unsafe fn _mm_blendv_values(a: uint32x4_t, b: uint32x4_t, mask: uint32x4_t) -> uint32x4_t {
            vbslq_u32(mask, b, a)
        }
